...
function ConfectioneryManagement() {
  ...
  const handleSaveProduct = async (productData) => {
    try {
      ...
      if (editingProduct) {
        ...
        setProducts(products.map((product) => (product.id === editingProduct.id ? savedProduct : product)))
      } else {
  ...
  const handleDeleteCategory = async (categoryId) => {
    const productsInCategory = products.filter((p) => p.category.id === categoryId)
  ...
  const handleSaveCategory = async (categoryData) => {
    try {
      ...
      if (editingCategory) {
        ...
        setCategories(categories.map((category) => (category.id === editingCategory.id ? savedCategory : category)))
      } else {
  ...
  const filteredProducts =
    selectedCategory === "all" ? products : products.filter((p) => p.category.id === Number.parseInt(selectedCategory))
  ...
  return (
    <AdminLayout>
      <div className="confectionery-management">
                  ...
                  <div className="product-info">
                    ...
                    <div className="product-details">
                      ...
                      <div className="product-stock">
                        <span
                          className={`stock-badge ${product.stock > 20 ? "high" : product.stock > 0 ? "low" : "empty"}`}
                        >
                          Stock: {product.stock} {product.stockUnit || "unidades"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {categories.map((category) => {
                    ...
                    return (
                      <tr key={category.id}>
                        ...
                        <td className="product-count">{productCount} productos</td>
        ...
        {showCategoryForm && (
          <CategoryForm category={editingCategory} onSave={handleSaveCategory} onClose={handleCloseCategoryForm} />
        )}
      </div>
    </AdminLayout>
  )

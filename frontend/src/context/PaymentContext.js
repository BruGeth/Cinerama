...
export const PaymentProvider = ({ children }) => {
    ...
    const createOrder = async (amountUSD) => {
        try {
            ...
            console.log("URL de aprobación cruda:", approvalUrl);

            // Validate approval URL
            if (!approvalUrl.startsWith("http")) {
                console.error("approval_url inválido:", approvalUrl);
                throw new Error("No se recibió una URL de aprobación válida de PayPal.");
            }

            // Extract paymentId from approval URL
            const paymentId = new URL(approvalUrl).searchParams.get("token");
            if (!paymentId) throw new Error("No se pudo extraer el paymentId del approval_url");
